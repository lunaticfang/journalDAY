...
function buildSubmissionEmailRecipients(manuscript, userEmail) {
...
export async function sendSubmissionReceiptEmail({
  ...
  const { authors, recipients: toList } = buildSubmissionEmailRecipients(
    manuscript,
    userEmail
  );
  if (!toList.length) {
...
export async function sendRevisionReceiptEmail({
  manuscript,
  version,
  userEmail,
  req,
  ...
  const { recipients: toList } = buildSubmissionEmailRecipients(manuscript, userEmail);
  if (!toList.length) {
    throw new Error("No recipient emails found for this revision.");
  }

  const title = manuscript?.title || "Manuscript";
  const updatedAtIso = formatSubmissionTimestamp(version?.created_at);
  const dashboardUrl = `${getAppBaseUrl(req)}/author/dashboard`;
  const subject = `Revision received: ${title}`;

  const html = `
    <div style="font-family: Arial, sans-serif; color: #111827;">
      <h2 style="color: #6A3291; margin-bottom: 12px;">Revision received</h2>
      <p>Your revised manuscript has been received by UpDAYtes.</p>
      <div style="margin: 18px 0; padding: 16px; border: 1px solid #e5d8f2; border-radius: 12px; background: #faf7fe;">
        <p style="margin: 0 0 10px;"><strong>Submission ID:</strong> ${escapeHtml(manuscript?.id)}</p>
        <p style="margin: 0 0 10px;"><strong>Latest version ID:</strong> ${escapeHtml(version?.id)}</p>
        <p style="margin: 0 0 10px;"><strong>Received at:</strong> ${escapeHtml(updatedAtIso)} (UTC)</p>
        <p style="margin: 0;"><strong>Title:</strong> ${escapeHtml(title)}</p>
      </div>
      <p>Only the latest version is now shown in your manuscript view.</p>
      <p style="margin: 18px 0;">
        <a
          href="${escapeHtml(dashboardUrl)}"
          style="display: inline-block; padding: 10px 14px; border-radius: 999px; background: #6A3291; color: #ffffff; text-decoration: none; font-weight: 600;"
  ...
  const text = [
    "Revision received",
    "",
    "Your revised manuscript has been received by UpDAYtes.",
    `Submission ID: ${manuscript?.id}`,
    `Latest version ID: ${version?.id}`,
    `Received at (UTC): ${updatedAtIso}`,
    `Title: ${title}`,
    "",
    "Only the latest version is now shown in your manuscript view.",
    `Open your submissions: ${dashboardUrl}`,
    "",
    "- Editorial Office",
  ].join("\n");
  ...
  const providerResponse = await sendTransactionalEmail({
    ...
    html,
    text,
    tags: ["submission", "revision-receipt"],
  });
