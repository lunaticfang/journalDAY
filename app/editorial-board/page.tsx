...
export default function EditorialBoardPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const membersResp = await fetch("/api/editorial-board");
      const membersJson = await membersResp.json().catch(() => ({}));

      if (!membersResp.ok) {
        console.error(
          "Load editorial board error:",
  ...
  if (loading) {
    return <p className="board-page__loading">Loading editorial board...</p>;
  }

  return (
    <main className="board-page">
      <h1 className="board-page__title">Editorial Board</h1>
